...
function showImportSummary(onConfirm) {
  ...
  // Every row starts pre-selected when a CSV loads — a vendor who never
  // deliberately picked rows (e.g. only meant to apply a markup to a
  // handful of items) can hit Import with everything still selected by
  // default and not notice, updating their entire catalog at once. Force
  // an explicit "yes, all of them" tick in that case rather than letting
  // Import Now be clicked straight through — small imports (≤5 rows,
  // where "all of them" is obviously the point) skip this extra step.
  const nothingDeselected = _selectedRows.size === _rows.length;
  const needsAllConfirm   = nothingDeselected && totalRows > 5;
  ...
  if (needsAllConfirm) {
    body += `<p style="margin:12px 0 0;padding:10px 12px;background:#fff7ed;border:1px solid #fdba74;border-radius:8px;color:#9a3412;font-size:13px">
      No rows were deselected — this will update <strong>all ${totalRows} products</strong> in this file, including price/markup. If you only meant to change some of them, cancel and select rows first.
    </p>
    <label style="display:flex;align-items:flex-start;gap:8px;margin-top:10px;font-size:13px;color:#374151;cursor:pointer">
      <input type="checkbox" id="import-confirm-all" style="width:auto;margin-top:2px" />
      Yes, update all ${totalRows} products
    </label>`;
  }
  ...
  if (needsAllConfirm) {
    ...
    confirmBtn.style.cssText += 'opacity:.5;cursor:not-allowed';
    modal.querySelector('#import-confirm-all').addEventListener('change', (e) => {
      confirmBtn.disabled = !e.target.checked;
      confirmBtn.style.opacity = e.target.checked ? '1' : '.5';
