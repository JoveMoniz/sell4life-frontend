...
function addVariantRow(data) {
  ...
  if (uploadBtn && uploadInput) {
    ...
    uploadInput.addEventListener('change', async () => {
      ...
      uploadBtn.innerHTML = window.s4lIcon ? window.s4lIcon('camera') : '';
...
function bindVariants() {
  ...
  vbMultiInput?.addEventListener('change', async (e) => {
    ...
    btnVariantImgs.innerHTML = '<svg class="s4l-icon" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-4px;flex-shrink:0;"><path d="M3 7a2 2 0 012-2h4l2 2h8a2 2 0 012 2v8a2 2 0 01-2 2H5a2 2 0 01-2-2V7z"/></svg> Bulk upload images';
    vbMultiInput.value = '';
...
async function loadProduct() {
  ...
  try {
    ...
    // "Charge for shipping" (the checkbox, on professional/enterprise, or the
    // Free/Charge/Collection button grid, on casual/refurbished) used to be
    // purely decorative here — nothing ever read its state on save or set it
    // on load, so it always rendered unticked regardless of the real setting,
    // and only the raw shippingCost figure actually drove checkout. shipIncluded
    // (from the backend, authoritative for what checkout actually charges) is
    // now used to derive and restore the real mode on load.
...
if (form) {
  form.addEventListener('submit', async (e) => {
    ...
    const product = {
      ...
      // it; the "Charge for shipping" checkbox/button grid used to have no
...
function reset(btn) {
  ...
  if (floatBtn) {
    ...
    const lbl = floatBtn.querySelector('.ep-save-label');
    if (lbl) lbl.innerHTML = `${window.s4lIcon ? window.s4lIcon('save') : ''} Save`;
  }
}
